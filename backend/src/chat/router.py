...
# from .schemas import ChannelCreate, ChannelResponse, ChannelDetailResponse, MessageCreate, MessageResponse
...
#     new_channel = await Channel.create(**channel.dict(), created_by=current_user)
...
#             created_by=member.channel.created_by.id if member.channel.created_by else None
#         )
...
#     channel = await Channel.get_or_none(id=channel_id).prefetch_related('members__user', 'messages__user')
#     if not channel:
...
#     return ChannelDetailResponse.from_orm(channel)
...
#     new_message = await Message.create(channel=channel, user=current_user, **message.dict())
