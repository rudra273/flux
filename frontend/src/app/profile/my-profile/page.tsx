...
import ProfileHeader from '../../../components/ProfileHeader';
import PostCard from '../../../components/PostCard';
import EditProfile from '../../../components/EditProfile'; // Changed here

export default function MyProfile() {
  const router = useRouter();
  const { user, isAuthenticated, isLoading } = useAuth();
  const [profile, setProfile] = useState<Profile | null>(null);
  const [isEditOpen, setIsEditOpen] = useState(false); // Changed here
  const [error, setError] = useState('');
  ...
  const handleUpdateProfile = async (data: ProfileUpdateData) => {
    ...
    try {
      await updateProfile(user.username, data);
      await fetchProfile();
      setIsEditOpen(false); // Changed here
    } catch (error) {
      setError('Failed to update profile');
      console.error(error);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <ProfileHeader
        profile={profile}
        isOwnProfile={true}
        onEditClick={() => setIsEditOpen(true)} // Changed here
      />
      ...
      {isEditOpen && ( // Changed here
        <EditProfile // Changed here
          profile={profile}
          onClose={() => setIsEditOpen(false)} // Changed here
          onSubmit={handleUpdateProfile}
        />
      )}
    </div>
